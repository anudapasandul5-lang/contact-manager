...
it("buildCompanyClusterGraph creates company-scoped projections for employees and merged vendor contacts", () => {
...
it("buildCompanyClusterGraph projects vendor contacts per company like other company members", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
...
it("buildCompanyClusterGraph creates company-scoped vendor business nodes for shared vendors", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
...
it("buildCompanyClusterGraph links projected contacts to their projects and supports shared company copies", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
  const aliceAlphaId = createCompanyMemberNodeId("company-1", "alice");
  const aliceBetaId = createCompanyMemberNodeId("company-2", "alice");
...
it("buildCompanyClusterGraph creates standalone-project projections for employee and vendor contacts without companies", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
...
it("buildCompanyClusterGraph tucks standalone vendor businesses into standalone projects", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
...
it("buildCompanyClusterGraph does not create project projections for company-owned project members", () => {
  const graph = buildCompanyClusterGraph(createNetworkData());
...
it("center node receives photoUrl and displayName from currentUser", () => {
...
it("center node data is null when currentUser is absent", () => {
  const data: NetworkData = {
    contacts: [],
    companies: [],
    vendors: [],
