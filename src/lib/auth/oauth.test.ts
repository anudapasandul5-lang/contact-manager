...
it("buildGoogleCallbackUrl appends the callback path to the request origin", () => {
  assert.equal(
...
it("buildLoginErrorRedirectUrl sends users back to login with a stable error code", () => {
  assert.equal(
...
it("getOAuthErrorMessage maps supported query codes to readable copy", () => {
  assert.equal(
  ...
  assert.equal(getOAuthErrorMessage("toString"), null);
});
