...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response = NextResponse.json(data.introRequests);
    applySessionCookies(response, auth.resolved);
    return response;
  } catch (error) {
    const message = normalizeIntroRequestError(error instanceof Error ? error.message : "Failed to load intro requests.");
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message = normalizeIntroRequestError(error instanceof Error ? error.message : "Failed to save intro request.");
