-- drizzle/migrations/20260415_fix_rpc_text_types.sql
-- Fix RPC function parameter types from uuid -> text to match the text-typed id columns.
-- Must DROP first because changing parameter types requires it.

DROP FUNCTION IF EXISTS create_contact_with_relations(uuid, uuid, text, text, text, text, text, text, text, uuid[], uuid[]);
DROP FUNCTION IF EXISTS update_contact_with_relations(uuid, uuid, text, text, text, text, text, text, text, uuid[], uuid[]);
DROP FUNCTION IF EXISTS create_vendor_with_relations(uuid, uuid, text, text, text, text, uuid[], uuid[], jsonb);
DROP FUNCTION IF EXISTS update_vendor_with_relations(uuid, uuid, text, text, text, text, uuid[], uuid[], jsonb);
...
BEGIN
  ...
  FOR v_person IN SELECT * FROM jsonb_array_elements(p_people) LOOP
    ...
    VALUES (
      COALESCE(v_person->>'id', gen_random_uuid()::text),
      p_id,
