-- drizzle/migrations/20260415_fix_rpc_mixed_types.sql
-- id columns are text (Drizzle-created), user_id columns are uuid (added via ALTER TABLE migration).
-- Params: p_id/array ids = text, p_user_id = uuid.

DROP FUNCTION IF EXISTS create_contact_with_relations(text, text, text, text, text, text, text, text, text, text[], text[]);
DROP FUNCTION IF EXISTS update_contact_with_relations(text, text, text, text, text, text, text, text, text, text[], text[]);
DROP FUNCTION IF EXISTS create_vendor_with_relations(text, text, text, text, text, text, text[], text[], jsonb);
DROP FUNCTION IF EXISTS update_vendor_with_relations(text, text, text, text, text, text, text[], text[], jsonb);
